...
public interface CustomerService {
    ...
    Single<Response<DataBracketResponse<HandymanDetailResponse>>> getHandymanDetail(@Header("Authorization") String token, @Body HandymanDetailRequest handymanDetailRequest);

    @GET(Constants.GET_CUSTOMER_INFO)
    Single<Response<DataBracketResponse<Customer>>> getCustomerInfo(@Header("Authorization") String token);

    @GET(Constants.GET_CUSTOMER_PROFILE)
    Single<Response<DataBracketResponse<CustomerProfileResponse>>> getCustomerProfile(@Header("Authorization") String token);

    @POST(Constants.EDIT_CUSTOMER_PROFILE)
    Single<Response<StandardResponse>> editCustomerProfile(@Header("Authorization") String token, @Query("customerEditName") String customerEditName);
}
